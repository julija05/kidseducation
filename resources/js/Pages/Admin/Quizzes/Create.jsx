...
export default function CreateQuiz({ programs, lessons, quiz_types }) {
    ...
    const availableLevels = data.program_id 
        ? [...new Set(lessons.filter(l => String(l.program_id) === String(data.program_id)).map(l => l.level))].sort((a, b) => a - b)
        : [];
    
    // Filter lessons based on selected program and level
    const availableLessons = lessons.filter(lesson => {
        if (!data.program_id) return false;
        if (String(lesson.program_id) !== String(data.program_id)) return false;
        if (data.level && String(lesson.level) !== String(data.level)) return false;
        return true;
    });
                ...
                <div className="md:flex md:items-center md:justify-between mb-6">
                    <div className="flex-1 min-w-0">
                        ...
                        <p className="mt-1 text-sm text-gray-500">
                            Set up a new quiz for your lesson
                        </p>
                    </div>
                </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    <div className="bg-white shadow rounded-lg">
                        ...
                        <div className="px-6 py-4 space-y-4">
                            {/* Selection Summary */}
                            {(data.program_id || data.level) && (
                                <div className="bg-blue-50 border border-blue-200 rounded-md p-3">
                                    <div className="text-sm text-blue-700">
                                        <strong>Selection:</strong> {' '}
                                        {data.program_id && (
                                            <span>{programs.find(p => p.id == data.program_id)?.name}</span>
                                        )}
                                        {data.level && (
                                            <span> → Level {data.level}</span>
                                        )}
                                        {data.lesson_id && (
                                            <span> → {availableLessons.find(l => l.id == data.lesson_id)?.title}</span>
                                        )}
                                    </div>
                                </div>
                            )}
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                ...
                                <FormField
                                    ...
                                    helper={!data.program_id ? "Select a program first" : `${availableLevels.length} levels available`}
                                    required
                                >
                                ...
                                <FormField
                                    ...
                                    disabled={!data.level}
                                    helper={!data.level ? "Select a level first" : `${availableLessons.length} lessons available`}
                                    required
                                >
                    ...
                    {data.type === 'mental_arithmetic' && data.lesson_id && (
                        <div className="bg-white shadow rounded-lg">
                            <div className="px-6 py-4 border-b border-gray-200">
